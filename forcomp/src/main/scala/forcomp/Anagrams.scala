...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    Nil :: (for {
      ...
      i <- 1 to count
      remainder <- combinations(occurrences.filter(char < _._1))
    } yield List((char, i)) ::: remainder)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    for {
      (char, count) <- x
      newCount = count - map.getOrElse(char, 0)
      if (newCount > 0)
    } yield (char, newCount)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def generate(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
      else
        for {
          combination <- combinations(occurrences) if !combination.isEmpty
          word <- dictionaryByOccurrences.getOrElse(combination, List())
          sentence <- generate(subtract(occurrences, wordOccurrences(word)))
        } yield word :: sentence
    }
    generate(sentenceOccurrences(sentence))
  }
}
