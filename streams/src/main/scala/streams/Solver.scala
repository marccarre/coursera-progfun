...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = b.legalNeighbors match {
    case Nil => Stream.Empty
    case neighbors => {
      for ((block, moves) <- neighbors)
        yield (block, moves :: history)
    }.toStream
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case(block, _) #:: tail if done(block) => Stream(initial.head)
    case _ => {
      ...
      initial #::: from(newNeighbors, explored ++ newNeighbors.map({ case(block, _) => block }))
    }
  }
